...
class ByJsonFile {
   ...
   * @return null
   */
  public function render(stdClass $routine) {
    if (count($routine) != 1) {
    ...
    foreach ($routine as $className => $row) {
      foreach ($row as $methodName => $object) {
        if (!empty($object->database) && !empty($object->variable)) {
          $class = $this->factoryClass($className, $methodName, $object->database);
   ...
   */
  private function factoryClass(string $className, string $methodName, stdClass $database): Routine {
    ...
    $routine = new $className($info, $pdo);
    return $routine;
  }
